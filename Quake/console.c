...
static qboolean CompleteLoad(const char* partial, void* unused)
{
	...
	SearchSaveFiles(save_dir, partial);
...
void Con_DrawNotify(void)
{
	...
	for (i = con_current - maxlines + 1; i <= con_current; i++) // woods from proquake 493 #notifylines
	{
		...
		{
			//avião: using fonts
			Uint32 codepoint = utf8_decode_nth(text, x, con_linewidth);
...
void Con_DrawInput(void)
{
	...
	if (key_tabhint[0])
	{
		...
		{
			//avião: using fonts
			Uint32 codepoint = utf8_decode_nth(key_tabhint, i, 256);
			Draw_CharacterRGBA((i + 1 + len - ofs) << 3, vid.conheight - 16, codepoint/* | 0x80*/, CL_PLColours_Parse("0x787878"), 0.75f);
		}
	}
