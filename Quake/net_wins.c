...
{
	if (tryEncodeSteamId(name, addr) == 0) {
		return 0;
	}
...
int WINS_AddrCompare(struct qsockaddr* addr1, struct qsockaddr* addr2)
{
	...
	if (isSteamAddr(addr1) && isSteamAddr(addr2))
	{
		...
		if (id1 == 0 || id2 == 0)
			return -1;   // treat invalids as non-equal

		if (id1 == id2)
			return 0;    // same SteamID -> same peer

		return -1;       // different SteamID -> different peer
	}
	else if (isSteamAddr(addr1) || isSteamAddr(addr2))
	{
		// One is Steam-style, the other is normal IP: they're never equal.
		return -1;
	}
	...
	if (addr1->qsa_family == AF_INET6)
	{
		if (memcmp(&((struct sockaddr_in6*)addr1)->sin6_addr,
			...
			sizeof(((struct sockaddr_in6*)addr2)->sin6_addr)))
			return -1;
